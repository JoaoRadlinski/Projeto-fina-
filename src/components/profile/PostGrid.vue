<template>
  <div class="post-grid">
    <RouterLink v-for="post in posts" :key="post.id" :to="`/posts/${post.id}`" class="post-grid__item">
      <img :src="post.image_url" :alt="post.caption || 'Post'" class="post-grid__img" loading="lazy" />
...
.post-grid__item { position: relative; aspect-ratio: 1; overflow: hidden; background: var(--color-surface-alt); }
.post-grid__img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.3s ease; }
.post-grid__overlay {
...
.post-grid__item:hover .post-grid__img { transform: scale(1.04); }
.post-grid__stat { display: flex; align-items: center; gap: 6px; color: #fff; font-size: 14px; font-weight: 700; }
.post-grid__empty {

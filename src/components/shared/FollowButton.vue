<template>
  <button
    class="follow-btn"
    :class="{ 'follow-btn--following': localFollowing, 'follow-btn--loading': loading }"
    :disabled="loading"
    @click.stop="toggle"
  >
    <span v-if="loading" class="follow-btn__spinner"></span>
    <template v-else>{{ localFollowing ? 'Seguindo' : 'Seguir' }}</template>
  </button>
</template>
...
.follow-btn--loading { opacity: 0.7; cursor: not-allowed; }
.follow-btn__spinner {

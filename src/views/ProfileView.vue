...
async function loadProfile() {
  ...
  try {
    ...
    profileError.value = err.response?.status === 404 ? 'Perfil não encontrado.' : 'Erro ao carregar perfil.'
  } finally {
    loading.value = false
  }
}
...
.profile-view__container { max-width: 600px; margin: 0 auto; }
.profile-view__loader { display: flex; justify-content: center; padding: 80px 0; }
...
.profile-view__error { text-align: center; padding: 80px 24px; color: var(--color-text-muted); }
.profile-view__back { display: inline-block; margin-top: 12px; color: var(--color-accent); font-weight: 600; text-decoration: none; }
.profile-view__back:hover { text-decoration: underline; }
</style>
